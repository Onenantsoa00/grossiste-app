<template>
  <q-page class="q-pa-md">
    <div class="row items-center q-mb-md">
      ...
      <q-btn color="primary" icon="add" label="Nouvel employé" @click="openDialog()" />
    </div>

    <q-table :rows="rows" :columns="columns" row-key="id" :loading="loading" flat bordered>
      <template #body-cell-actif="props">
        <q-td :props="props">
          <q-badge :color="props.row.actif ? 'positive' : 'negative'" :label="props.row.actif ? 'Actif' : 'Bloqué'" />
        </q-td>
      </template>
      <template #body-cell-actions="props">
        <q-td :props="props">
          <q-btn flat dense icon="edit" color="primary" @click="openDialog(props.row)" />
          <q-btn flat dense icon="lock_reset" color="orange" @click="resetPassword(props.row)" />
    ...
    <q-dialog v-model="dialog">
      <q-card style="min-width: 400px">
        <q-card-section class="text-h6">{{ form.id ? 'Modifier' : 'Créer' }} un employé</q-card-section>
        <q-card-section class="q-gutter-md">
          ...
          <q-select
            ...
            map-options
          />
          <q-input v-if="!form.id" v-model="form.password" label="Mot de passe *" type="password" outlined />
...
export default {
  ...
  setup () {
    ...
    const columns = [
      { name: 'nom', label: 'Nom', field: row => `${row.prenom || ''} ${row.nom}`, align: 'left' },
    ...
    async function load () {
      ...
      try {
        ...
        grossisteOptions.value = grossistes.data.map(g => ({ label: g.nom, value: g.id }))
      } finally {
        loading.value = false
      }
    }
    ...
    function openDialog (row = null) {
      form.value = row
        ...
        : { nom: '', prenom: '', email: '', telephone: '', password: '', grossiste_id: null, actif: true }
      dialog.value = true
    }
    ...
    return { rows, columns, grossisteOptions, loading, saving, dialog, form, openDialog, save, resetPassword, confirmDelete }
  }
}
</script>
