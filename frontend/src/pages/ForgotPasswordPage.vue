<template>
  <q-page class="flex flex-center bg-grey-2">
    <q-card class="q-pa-lg auth-card">
      ...
      <q-card-section>
        <q-form v-if="!resetLink" @submit.prevent="onSubmit" class="q-gutter-md">
        ...
        <div v-else class="q-gutter-md">
          ...
          <div v-if="resetLink" class="text-body2">
            ...
            <q-btn color="primary" label="Réinitialiser maintenant" class="full-width q-mt-md" :to="resetRoute" />
          </div>
          <div class="text-center">
            <router-link to="/login" class="text-primary">Retour connexion</router-link>
          </div>
...
export default {
  ...
  setup () {
    ...
    const resetRoute = computed(() => {
      ...
      if (resetLink.value) {
        try {
          ...
          return { path: '/reset-password', query: { token: url.searchParams.get('token') } }
        } catch {
    ...
    return { email, loading, resetLink, devToken, resetRoute, onSubmit, copyLink }
  }
}
</script>
