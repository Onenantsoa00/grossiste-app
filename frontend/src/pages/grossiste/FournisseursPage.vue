<template>
  <q-page class="q-pa-md">
    <div class="row items-center q-mb-md">
      <div class="text-h5 col">Fournisseurs</div>
    ...
    <q-dialog v-model="dialog">
      <q-card style="min-width: 400px">
        <q-card-section class="text-h6">{{ form.id ? 'Modifier' : 'Ajouter' }} fournisseur</q-card-section>
...
export default {
  ...
  setup () {
    ...
    async function save () {
      saving.value = true
      try {
        if (form.value.id) await api.put(`/fournisseurs/${form.value.id}`, form.value)
    ...
    function remove (row) {
      $q.dialog({ title: 'Supprimer', message: `Supprimer ${row.nom} ?`, cancel: true })
        .onOk(async () => { await api.delete(`/fournisseurs/${row.id}`); await load() })
    }
