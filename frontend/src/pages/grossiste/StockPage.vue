          ...
          <q-item v-for="a in alertes" :key="a.id">
            <q-item-section>
              ...
              <q-item-label caption>
                Stock: {{ formatQuantite(a.stock, a.unite_vente) }}
                {{ uniteLabel(a.unite_vente) }} / Min:
                {{ formatQuantite(a.stock_min, a.unite_vente) }}
              </q-item-label>
            </q-item-section>
            <q-item-section side>
...
const TYPE_HELP = {
  entree: {
    ...
    description:
      "Augmente le stock. Utilisez ce type quand vous recevez des produits (achat fournisseur, livraison). Indiquez le montant dépensé pour suivre vos coûts d'achat.",
  },
  sortie: {
    title: "Sortie — Retrait manuel",
    description:
      "Diminue le stock sans vente. Exemples : produit cassé, périmé jeté, échantillon offert, perte ou vol constaté.",
  },
  inventaire: {
    title: "Inventaire — Comptage physique",
    description:
      "Remplace le stock par la quantité réelle comptée. Exemple : après inventaire annuel, il reste exactement 230 pièces en rayon.",
  },
  ajustement: {
    title: "Ajustement — Correction",
    description:
      "Corrige une erreur de saisie ou un écart constaté. Comme l'inventaire, définit la nouvelle valeur exacte du stock (produit retrouvé, erreur corrigée).",
  },
};
  ...
  setup() {
    ...
    const mvColumns = [
      ...
      },
      {
        ...
        label: "Coût",
        field: (row) =>
          row.cout_achat > 0 ? formatMontant(row.cout_achat) : "—",
        align: "right",
      },
    ...
    const typeHelp = computed(
      () => TYPE_HELP[form.value.type] || TYPE_HELP.entree,
    );
    ...
    const quantiteLabel = computed(() => {
      ...
      if (!p) return "Quantité / Kg";
      if (
        form.value.type === "inventaire" ||
        form.value.type === "ajustement"
      ) {
        return isKg(p.unite_vente)
          ? "Nouveau stock réel (kg)"
          : "Nouveau stock réel (unités)";
      }
