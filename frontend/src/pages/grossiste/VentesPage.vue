    ...
    <div class="row items-center q-mb-md q-col-gutter-sm">
      ...
      <div class="col-12 col-sm-6 col-md-4">
        <q-input
          ...
          clearable
          debounce="300"
          @update:model-value="onSearchRecu"
        >
        ...
        <div class="q-pa-xs col-12">
          <q-card bordered flat class="q-pa-sm">
            <div class="text-caption text-grey-7">
              Reçu : {{ props.row.recu || props.row.numero }}
            </div>
            <div class="text-subtitle2">
              {{ new Date(props.row.date).toLocaleString("fr-FR") }}
            </div>
            <div>{{ props.row.client_nom || "Sans client" }}</div>
            <div class="text-h6 q-mt-xs">
              {{ formatMontant(props.row.total) }}
            </div>
            <q-btn
              flat
              dense
        ...
        <q-card-section class="row q-col-gutter-md">
                  ...
                  <template #no-option>
                    <q-item>
                      <q-item-section class="text-grey"
                        >Aucun produit trouvé — tapez pour
                        rechercher</q-item-section
                      >
                    </q-item>
                  </template>
                </q-select>
            ...
            >
              ...
              <template #body-cell-prix="props">
                <q-td :props="props">
                  ...
                  }}{{ unitePrixSuffix(props.row.unite_vente) }}
                </q-td>
              </template>
              ...
              <template #body-cell-actions="props">
                <q-td :props="props">
                  <q-btn
                    ...
                    icon="delete"
                    color="negative"
                    @click="removeFromPanier(props.row)"
                  />
                </q-td>
              </template>
              <template #no-data>
          ...
          <div class="col-12 col-md-5">
            ...
            <q-card
              ...
              :class="monnaieARendre > 0 ? 'bg-amber-1' : 'bg-grey-2'"
            >
              <div class="row justify-between items-center">
                <span class="text-subtitle1">Monnaie à rendre</span>
                <span
                  class="text-h5 text-weight-bold"
                  :class="monnaieARendre > 0 ? 'text-amber-10' : 'text-grey-7'"
                >
                  {{ formatMontant(monnaieARendre) }}
                </span>
              </div>
              <div
                v-if="montantRemis < total"
      ...
      <q-card style="min-width: 360px">
        ...
        <q-card-section v-if="produitToAdd" class="q-gutter-md">
          ...
          <div class="text-body2">
            Prix sélectionné :
            {{ formatMontant(prixSelectionne)
            }}{{ unitePrixSuffix(produitToAdd.unite_vente) }}
          </div>
          <div
            class="text-caption text-grey-7"
            v-if="conversionHint(produitToAdd)"
          >
          ...
          <div class="text-caption text-grey-7">
            ...
            {{
              formatQuantite(
                stockDisponible(produitToAdd, addTypePrix),
                produitToAdd.unite_vente,
              )
            }}
            {{ uniteLabel(produitToAdd.unite_vente) }}
            <span v-if="addTypePrix === 'carton' || addTypePrix === 'sac'">
              (max {{ maxVendableDispo(produitToAdd) }}
              {{ labelVenteType(addTypePrix) }})
            </span>
          </div>
          <q-input
          ...
          <div class="text-body2 text-right">
            Ligne :
            <strong>{{
              formatMontant(prixSelectionne * (addQty || 0))
            }}</strong>
          </div>
        </q-card-section>
    ...
    <q-dialog v-model="showDetailDialog" :maximized="$q.screen.xs">
      <q-card class="dialog-card">
        <q-card-section class="row items-center">
          <div class="col">
            ...
            <div v-if="venteDetail" class="text-caption text-grey-7">
              Reçu :
              <strong>{{ venteDetail.recu || venteDetail.numero }}</strong> —
        ...
        <q-card-section v-else-if="venteDetail">
          <div class="q-mb-md">
            <div>
              <strong>Client :</strong> {{ venteDetail.client_nom || "—" }}
            </div>
            <div>
              <strong>Total :</strong> {{ formatMontant(venteDetail.total) }}
            </div>
            <div v-if="venteDetail.remise > 0">
              <strong>Remise :</strong> {{ formatMontant(venteDetail.remise) }}
            </div>
          </div>
          <q-table
  ...
  setup() {
    ...
    const columns = [
      ...
      },
      {
        name: "total",
        label: "Total",
        field: (row) => formatMontant(row.total),
        align: "right",
      },
    ...
    const panierColumns = [
      ...
      { name: "qty", label: "Quantité", field: "quantite", align: "center" },
      {
        name: "total",
        label: "Total",
        field: (row) => formatMontant(row.prix * row.quantite),
        align: "right",
      },
      { name: "actions", label: "", field: "actions", align: "center" },
    ];
    ...
    const detailColumns = [
      ...
      },
      {
        name: "type_prix",
        label: "Tarif",
        field: (row) => labelTypePrix(row.type_prix),
        align: "center",
      },
      {
        name: "prix",
        label: "Prix",
        field: (row) => formatMontant(row.prix),
        align: "right",
      },
    ...
    const addTypePrixOptions = computed(() => {
      ...
      return opts.filter((o) => {
        ...
        if (o.value === "grossiste")
          return parseFloat(p.prix_grossiste || 0) > 0;
        if (o.value === "carton")
          return (
            parseFloat(p.prix_carton || 0) > 0 &&
            parseFloat(p.qte_par_carton || 0) > 0
          );
        if (o.value === "sac")
          return (
            parseFloat(p.prix_sac || 0) > 0 &&
            parseFloat(p.qte_par_sac || 0) > 0
          );
        return false;
      });
    });
    ...
    const addQtyLabel = computed(() => {
      ...
      return isKg(produitToAdd.value.unite_vente)
        ? "Quantité (kg)"
        : "Quantité (unités)";
    });

    const addQtyStep = computed(() => {
      if (addTypePrix.value === "carton" || addTypePrix.value === "sac")
        return 1;
    ...
    async function load(newRecu = null) {
      ...
      try {
        const params = {};
        const query =
          newRecu !== null ? String(newRecu).trim() : searchRecu.value.trim();
        if (query) params.recu = query;
    ...
    function confirmAddToPanier() {
      ...
      if (prix <= 0) {
        $q.notify({
          type: "warning",
          message: "Le prix sélectionné doit être supérieur à 0",
        });
        return;
      }

      const qty =
        addTypePrix.value === "carton" || addTypePrix.value === "sac"
          ? Number.isInteger(addQty.value) && addQty.value > 0
            ? addQty.value
            : null
    ...
    function updateQty(row, val) {
      ...
      if (qty == null) {
        $q.notify({
          type: "warning",
          message: isKg(row.unite_vente)
            ? "Quantité kg invalide"
            : "Quantité entière requise",
        });
        return;
      }
    ...
    function removeFromPanier(row) {
      panier.value = panier.value.filter(
        (l) =>
          !(l.produit_id === row.produit_id && l.type_prix === row.type_prix),
      );
    }
    ...
    async function validerVente() {
      ...
      for (const ligne of panier.value) {
        ...
        if (ligne.quantite <= 0 || totalDed > ligne.stock) {
          $q.notify({
            type: "warning",
            message: `Quantité invalide pour « ${ligne.nom} »`,
          });
          return;
        }
      }
      ...
      try {
        const montantPaye =
          montantRemis.value > 0
            ...
            : 0;
        const paiements =
          montantPaye > 0 ? [{ mode: "especes", montant: montantPaye }] : [];
        ...
        $q.notify({
          type: "negative",
          message: err.response?.data?.message || "Erreur lors de la vente",
        });
      } finally {
        saving.value = false;
      }
