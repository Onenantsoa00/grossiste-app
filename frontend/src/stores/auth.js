...
export const useAuthStore = defineStore("auth", {
  state: () => ({
    ...
    activeGrossiste: JSON.parse(
      localStorage.getItem("activeGrossiste") || "null",
    ),
    subscriptionWarning: false,
    subscriptionDaysLate: 0,
  ...
  getters: {
    ...
    activeGrossisteId: (state) =>
      state.activeGrossiste?.id || state.user?.grossiste_id || null,
  },
  ...
  actions: {
    ...
    setSubscriptionStatus({ subscriptionWarning, daysLate, expirationDate }) {
      this.subscriptionWarning = subscriptionWarning;
      this.subscriptionDaysLate = daysLate;
      this.subscriptionExpirationDate = expirationDate;
    },

    clearSubscriptionStatus() {
      this.subscriptionWarning = false;
      this.subscriptionDaysLate = 0;
