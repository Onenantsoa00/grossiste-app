...
export async function getBossDashboard(req, res, next) {
  try {
    ...
    stats.solde =
      parseFloat(stats.chiffre_affaires) -
      parseFloat(stats.total_decaisse || 0);
    ...
    const grossistes = grossistesResult.rows.map((g) => ({
      ...g,
      chiffre_affaires:
        parseFloat(g.ca_brut) - parseFloat(g.total_decaisse || 0),
  ...
  try {
    ...
    const statsResult = await pool.query(
      `SELECT
        ...
        (SELECT COALESCE(SUM(montant), 0)::numeric FROM decaissements WHERE grossiste_id = $1) AS total_decaisse,
